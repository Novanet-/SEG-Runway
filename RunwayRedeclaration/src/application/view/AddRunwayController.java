...
public class AddRunwayController {
	...
	@FXML
	private ComboBox cmbRunwayPosition;
	@FXML
	private TextField txtPrimaryTORA;
	@FXML
	private TextField txtPrimaryTODA;
	@FXML
	private TextField txtPrimaryASDA;
	@FXML
	private TextField txtPrimaryLDA;
	@FXML
	private TextField txtPrimaryDisplacedThreshold;
	@FXML
	private TextField txtSecondaryTORA;
	@FXML
	private TextField txtSecondaryTODA;
	@FXML
	private TextField txtSecondaryASDA;
	@FXML
	private TextField txtSecondaryLDA;
	@FXML
	private TextField txtSecondaryDisplacedThreshold;
	...
	private void handleBtnSubmitRunway() {
		try {
			...
			double primaryDisplacedThreshold = Double
					.parseDouble(txtPrimaryDisplacedThreshold.textProperty().getValue());
			...
			double secondaryDisplacedThreshold = Double
					.parseDouble(txtSecondaryDisplacedThreshold.textProperty().getValue());

			RunwayParameters primaryParameters, secondaryParameters;
			primaryParameters = new RunwayParameters(new SimpleDoubleProperty(primaryTORA),
					new SimpleDoubleProperty(primaryTODA), new SimpleDoubleProperty(primaryASDA),
					new SimpleDoubleProperty(primaryLDA), new SimpleDoubleProperty(primaryDisplacedThreshold));
			secondaryParameters = new RunwayParameters(new SimpleDoubleProperty(secondaryTORA),
					new SimpleDoubleProperty(secondaryTODA), new SimpleDoubleProperty(secondaryASDA),
			...
			Runway primaryRunway = new Runway(primaryParameters, null, new SimpleIntegerProperty(0),
					new SimpleStringProperty(
							cmbRunwayAlignment.getValue().toString() + cmbRunwayPosition.getValue().toString()));
			Runway secondaryRunway = new Runway(secondaryParameters, null, new SimpleIntegerProperty(1),
					new SimpleStringProperty(lblSecondRunwayAlignment.getText()));
